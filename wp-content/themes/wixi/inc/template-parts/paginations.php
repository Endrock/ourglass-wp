...
if ( ! function_exists( 'wixi_posts_prev_pag_attrs' ) ) {
    function wixi_posts_next_pag_attrs()
    {
        return 'class="nt-pagination-link -next"';
    }
    add_filter( 'next_posts_link_attributes', 'wixi_posts_next_pag_attrs' );
}
...
if ( ! function_exists( 'wixi_posts_prev_pag_attrs' ) ) {
    function wixi_posts_prev_pag_attrs()
    {
        return 'class="nt-pagination-link -previous"';
    }
    add_filter( 'previous_posts_link_attributes', 'wixi_posts_prev_pag_attrs' );
}
...
if ( ! function_exists( 'wixi_single_navigation' ) ) {
    function wixi_single_navigation()
    {
        if ( '0' != wixi_settings( 'single_navigation_visibility', '1' ) ) {
            ...
            <div class="pagination">
                <span><?php previous_post_link( '%link', _x('PREVIOUS POST', 'Previous post link', 'wixi' ) ); ?></span>
                <span class="icon"><a href="<?php echo esc_url( $blog ); ?>"><i class="fas fa-th-large"></i></a></span>
                <span class="text-right"><?php next_post_link( '%link', _x('NEXT POST', 'Next post link', 'wixi' ) ); ?></span>
            </div>
...
if ( ! function_exists( 'wixi_single_navigation_two' ) ) {
    function wixi_single_navigation_two()
    {
        if ( '1' == wixi_settings( 'single_navigation_visibility', '0' ) ) {
...
if ( ! function_exists( 'wixi_index_loop_pagination' ) ) {
    function wixi_index_loop_pagination()
    {
        if ( '0' !=  wixi_settings( 'pagination_visibility', '1' ) ) {
            $pag_class= array();
            $pag_class[] = ('yes' == wixi_settings( 'pag_group') ) ? '-group' : '';
            $pag_class[] = '-style-'.wixi_settings( 'pag_type', 'default' );
            $pag_class[] = '-size-'.wixi_settings( 'pag_size', 'medium' );
            $pag_class[] = '-corner-'.wixi_settings( 'pag_corner', 'square' );
            $pag_class[] = '-align-'.wixi_settings( 'pag_align', 'center' );
        ...
        <ul class='nt-pagination-inner'>" . "\n";
            ...
            if ( get_previous_posts_link() ) {
                echo '<li class="nt-pagination-item">' . wp_kses( $prev, wixi_allowed_html() ) . '</li>';
            }
            ...
            if ( get_next_posts_link() ) {
                echo '<li class="nt-pagination-item">' . wp_kses( $next, wixi_allowed_html() ) . '</li>';
            }
...
if ( ! function_exists( 'wixi_current_link_pages' ) ) {
    function wixi_current_link_pages( $link )
    {
        if ( ctype_digit( $link ) ) {
    ...
    add_filter( 'wp_link_pages_link', 'wixi_current_link_pages' );
}
...
if ( ! function_exists( 'wixi_wp_link_pages' ) ) {
    function wixi_wp_link_pages()
    {
        // pagination for page links
        wp_link_pages( array(
            'before' => '<div class="page-links"><span class="page-links-title">' . esc_html__( 'Pages', 'wixi' ) . '</span>',
            'after' => '</div>',
            'link_before' => '',
            'link_after' => '',
            'next_or_number' => 'number',
            'separator' => ' ',
            'nextpagelink' => esc_html__( 'Next page', 'wixi' ),
            'previouspagelink' => esc_html__( 'Previous page', 'wixi' ),
            'pagelink' => '%',
            'echo' => 1
        ));
    }

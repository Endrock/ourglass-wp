...
*/
if ( ! class_exists( 'Wixi_Wp_Bootstrap_Navwalker' ) ) {
    class Wixi_Wp_Bootstrap_Navwalker extends Walker_Nav_Menu
    {
        ...
        public static function fallback($args)
        {
            if (current_user_can('edit_theme_options')) {
                echo '<li><a href="' . admin_url('nav-menus.php') . '">' . esc_html__('Add a menu', 'wixi') . '</a></li>';
            }
        }
    }
}

if ( ! class_exists( 'Wixi_Menu_Navwalker' ) ) {
    class Wixi_Menu_Navwalker extends Walker_Nav_Menu
    {
        public function start_lvl(&$output, $depth = 0, $args = array())
        {
            $indent = str_repeat("\t", $depth);
            $submenu = ($depth > 0) ? '' : '';
            $close = esc_html__('Close','wixi');
            $output	.= "\n$indent<ul class=\"sub-menu depth_$depth\"><li class=\"menu-item goback\"><div class=\"hidden-item\"><a href=\"#0\"><i class=\"fas fa-times\"></i> $close</a></div></li>\n";
        }
        public function end_lvl(&$output, $depth = 0, $args = array())
        {
            $output	.= "</ul>";
        ...
        public function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0)
        {
            ...
            $number = 'default' == wixi_settings( 'header_template', 'default' ) ? '<span class="nm"></span>' : '';
            $link_class = apply_filters( 'header_link_class', 'link' );
            if ( $depth < 6 && $args->has_children ) {
                $linkitem ='<div class="hidden-item"><a '. $attributes .' class="'.$link_class.'">'.$number.$title.' <i class="fas fa-angle-down"></i></a></div>';
            } else {
                $linkitem  = '<div class="hidden-item"><a '. $attributes .' class="'.$link_class.'">'.$number.$title.' '.$desc.'</a></div>';
            }
